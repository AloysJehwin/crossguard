...
/**
 * @title CrossGuard Insurance Protocol
 * @notice Smart contract insurance on Somnia testnet
 * @dev Deployed to Somnia testnet for DeFi protocol protection
 */
contract CrossGuardInsurance is ReentrancyGuard, Ownable {
    // Native SOMNIA token interface (if using ERC20 wrapper)
    IERC20 public immutable somniaToken;
    ...
    // Constants
    uint256 public constant MIN_COVERAGE = 1000 * 10**18; // 1000 SOMNIA
    uint256 public constant MAX_COVERAGE = 10000000 * 10**18; // 10M SOMNIA
    ...
    constructor(address _somniaToken) Ownable(msg.sender) {
        somniaToken = IERC20(_somniaToken);
    }
    
    /**
     * @notice Purchase insurance for a smart contract
     * @param _protocolAddress Address of the contract to insure
     * @param _coverageAmount Amount of coverage in SOMNIA tokens
    ...
    function purchaseInsurance(
        ...
        require(
            somniaToken.transferFrom(msg.sender, address(this), premium),
    ...
    function approveClaim(uint256 _claimId) external onlyOwner nonReentrant {
        ...
        require(
            somniaToken.transfer(policy.holder, claim.claimAmount),
    ...
    function emergencyWithdraw() external onlyOwner {
        uint256 balance = somniaToken.balanceOf(address(this));
        require(somniaToken.transfer(owner(), balance), "Withdrawal failed");
    }
}